...
function handleClick() {
    if (!gameEnded && !this.textContent) {
        // Player's move
        this.textContent = 'X';
        const winnerAfterPlayerMove = checkWin();
        if (winnerAfterPlayerMove) {
            gameEnded = true;
            updateCountBoard(winnerAfterPlayerMove);
            showStrike(winningCombinations.find(combination => {
                const [a, b, c] = combination;
                return cells[a].textContent === winnerAfterPlayerMove && cells[b].textContent === winnerAfterPlayerMove && cells[c].textContent === winnerAfterPlayerMove;
            }));
            showModal(winnerAfterPlayerMove);
            return; // Exit function if player wins
        } else if ([...cells].every(cell => cell.textContent)) {
            gameEnded = true;
            showModal("It's a tie!"); // Show tie message
            return; // Exit function if it's a tie
        }
        
        // Computer's move
        generateComputerMove();
        const winnerAfterComputerMove = checkWin();
        if (winnerAfterComputerMove) {
            gameEnded = true;
            updateCountBoard(winnerAfterComputerMove);
            showStrike(winningCombinations.find(combination => {
                const [a, b, c] = combination;
                return cells[a].textContent === winnerAfterComputerMove && cells[b].textContent === winnerAfterComputerMove && cells[c].textContent === winnerAfterComputerMove;
            }));
            showModal(winnerAfterComputerMove);
        } else if ([...cells].every(cell => cell.textContent)) {
            gameEnded = true;
            showModal("It's a tie!"); // Show tie message
        }
...
function generateComputerMove() {
    // Disable cell clicks while the computer is making its move
    cells.forEach(cell => {
    ...
    // Check if the selected cell is empty, if not, generate another move
    if (!cells[randomIndex].textContent) {
        setTimeout(() => {
            // Set the computer's move
            cells[randomIndex].textContent = 'O'; // Set the computer's move text

            // Enable cell clicks after the computer's move
            cells.forEach(cell => {
            ...
            if (winnerAfterMove) {
                gameEnded = true;
                updateCountBoard(winnerAfterMove);
                showStrike(winningCombinations.find(combination => {
                    const [a, b, c] = combination;
        ...
        generateComputerMove(); // Recursive call to generate another move
